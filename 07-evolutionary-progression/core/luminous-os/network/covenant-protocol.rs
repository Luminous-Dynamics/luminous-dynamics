...
impl CovenantNetwork {
    ...
    async fn create_covenant(&self, id: CovenantId) -> Result<Arc<RwLock<Covenant>>, CovenantError> {
        ...
        let covenant = Covenant {
            id,
            participants: vec![handshake_data.initiator, handshake_data.responder],
...
struct FieldMonitor {
    monitoring_tasks: Arc<Mutex<HashMap<CovenantId, tokio::task::JoinHandle<()>>>>,
}
...
impl FieldMonitor {
    ...
    async fn monitor_covenant(&self, covenant_id: CovenantId) {
        ...
        self.monitoring_tasks.lock().unwrap().insert(covenant_id, handle);
    }
}
...
impl HandshakeManager {
    ...
    async fn initiate(
        ...
        let token = HandshakeToken(rand::random());
