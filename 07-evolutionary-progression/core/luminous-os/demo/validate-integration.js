...
// Test 6: Check WebSocket server health
...
await test('HTTP demo server', async () => {
    try {
        const response = await fetch('http://localhost:8000/luminous-os-demo.html');
        if (!response.ok) {
            throw new Error(`Demo server returned ${response.status}`);
        }
    } catch (error) {
        throw new Error(`Demo server not accessible: ${error.message}`);
    }
});
