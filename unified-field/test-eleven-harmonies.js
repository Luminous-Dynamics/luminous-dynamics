 ...
 * ES Module version for compatibility with true-integration-schema.js
 */

import { glyphs, appliedHarmonies } from './true-integration-schema.js';
...
${colors.reset}`);
  ...
  async testSchemaIntegrity() {
    ...
    try {
      if (glyphs && appliedHarmonies) {
        this.pass('Schema loads successfully');
        
        if (glyphs.foundations && appliedHarmonies.practices) {
  ...
  async testAllHarmoniesPresent() {
    ...
    try {
      const practices = appliedHarmonies.practices;
  ...
  async testMysticalBridges() {
    ...
    try {
      const practices = appliedHarmonies.practices;
      const foundations = glyphs.foundations;
  ...
  async testSacredSets() {
    ...
    try {
      const metadata = appliedHarmonies.metadata;
    ...
    try {
      ...
      const coherenceChecks = {
        'Mystical-Practical Balance': () => {
          return glyphs && appliedHarmonies;
        },
        ...
        'Integration Complete': () => {
          return appliedHarmonies.metadata && 
                 appliedHarmonies.metadata.integrationStatus === 'complete';
        }
      };
...
export default ElevenHarmoniesTestSuite;