...
impl GlyphInvoker {
    ...
    pub async fn invoke(&self, glyph_type: CoreGlyph, intention: String, user_coherence: f64) 
        ...
        self.active_vortices.lock().unwrap().insert(vortex_id, vortex);
    ...
    pub async fn send_intention(&self, vortex_id: VortexId, intention: UserIntention) 
        -> Result<IntentionResponse, GlyphError> {
        // Get vortex and glyph
        let vortex = self.active_vortices.lock().unwrap()
            .get(&vortex_id)
        ...
        // Process intention
        let response = glyph.process_intention(intention, 
            &mut self.active_vortices.lock().unwrap().get_mut(&vortex_id).unwrap()).await?;
